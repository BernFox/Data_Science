...
def class_insult(mini=2 ,maxi=50, folds=10, nmin=1, nmax=2, bin='b'):
	...
	all_fpr, all_tpr, all_auc, all_acc = (np.zeros(folds), np.zeros(folds), np.zeros(folds),np.zeros(folds))
	...
	vectorizer2 = CountVectorizer(min_df=mini, max_df=maxi, decode_error='ignore',stop_words=stopwords, vocabulary=tvocab, ngram_range=(nmin,nmax))
